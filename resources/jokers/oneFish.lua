...
local itemCfg = Collectables.utils.getItemConfig("Joker", "one_fish")
    ...
    calculate = function(self, card, context)
        ...
        then
            ...
            local hasAnotherSeussJoker = 
                #(SMODS.find_card("j_col_two_fish", true) or {}) >= 1
...
return {
    ...
    config = function() 
        return {
            key = "one_fish",
            name = localize("name_mod_one_fish", "mod_config"),
            stateTable = Collectables.loadState.items["Joker"]["one_fish"],
            refTable = Collectables.config.items["Joker"]["one_fish"],
            configItems = {
                {
                    type = "toggle",
                    refValue = "enabled",
